...
\newcommand{\trTitle}{Formulating a spatio-temporal approach to analyze longline survey data for the Atlantic Halibut fishery}
...
% Abstract
\newcommand{\trAbstract}{The Industry-DFO Atlantic halibut (\emph{Hippoglossus hippoglossus}) longline survey has been running since 1998 with a fixed stations design with the aim of giving better information on halibut abundance due to more appropriate gear than previous surveys. A review of modelling methods applied to this data found them to be inappropriate, which sparked the interest in utilizing a multinomial exponential model (MEM) to account for hook competition and the abundance of non-target species. This review further prompted the modification of the sampling design which led to the implementation of a stratified random sampling design in 2017. The most recent work done on this strictly focused on incorporating spatial patterns into the modelling process. The aim of this report is to present a new version of MEM that has been reworked into a fully spatio-temporal model. Utilizing a random walk structure on random temporal intercepts while accounting for the impact of individual vessel effects, this new formulation is able to successfully model the data from the fixed stations and the stratified random sampling design separately and together, allowing the inclusion of almost all available data into a unified framework. The analysis further illuminates potential issues with the fixed stations design and shows that the switch to a stratified random sampling design was very timely and helped better capture the increase in halibut catch rates around 2017.}

% Resume (i.e., French abstract)
\newcommand{\trResume}{Le relevé à la palangre de l'industrie et du MPO pour le flétan atlantique (\emph{Hippoglossus hippoglossus}) est en cours depuis 1998 utilisant des stations fixes de collection de données et ayant comme objectif d'obtenir de l'information plus approprié sur l'abondance du flétan avec l'utilisation de d'équipement de pêche plus approprié qu'auparavant. Un examen des modèles utilisés sur ces données les a trouvé inapropriés mais a identifié un modèle alternatif, le modèle multinomial exponentiel (MME). L'avantage de ce modèle réside dans sa capacité de prendre en compte l'abondance des espèces non cibles et la compétition pour les crochets de pêche. De plus, cet examen a incité la modification du protocole de collection de données avec l'implémentation d'un plan d'échantillonage aléatoire stratifié en 2017. Le travail le plus récent sur le MME s'est concentré sur l'inclusion de la répartition spatial du flétan à l'intérieur du modèle. L'objectif principal de ce rapport est la présentation d'une nouvelle version du MME qui inclut les changements spatiaux et temporels dans l'abondance du flétan atlantique. Utilisant une structure de marche aléatoire sur des interceptions aléatoires temporels tout en prenant en compte l'effect aléatoire des navires de pêche, cette nouvelle formulation est capable de modèler les données provenant des stations fixes et de l'échantillonage aléatoire stratifié séparemment et ensemble, ce qui permet l'inclusion de la majorité de l'information disponible à l'intérieur du même modèle. L'analyse souligne des problèmes provenant potentiellement du relevé d'échantillonage utilisant des stations fixes et démontre que l'implémentation du relevé d'échantillonage aléatoire stratifié fut opportun car il a permis au modèle de mieux capturer l'augmentation des taux de captures de flétan autour de 2017.}
...
\caption{Station-specific estimated target species cathc rate for all data between 2000 and 2021 fit to random walk model.}\label{fig:target-spat-fixed}
\end{figure}
\begin{figure}[htb]
