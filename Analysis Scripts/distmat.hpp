...
template <class Type>
matrix <Type> DistMat2D_2var(matrix <Type> locations1, matrix <Type> locations2){

  int m = locations1.col(0).size();
  int n = locations2.col(0).size();

  matrix <Type> distance(m,n);

  for (int i=0; i < m; i++){
    for (int j=0; j < n; j++) {
      distance(i,j) = sqrt(pow(locations1(i,0)-locations2(j,0),2)+pow(locations1(i,1)-locations2(j,1),2));
    }
  }
