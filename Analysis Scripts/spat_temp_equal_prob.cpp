...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER(theta_nt); // Parameters for pnt (probability of caught non-target fish escaping)
  ...
  // Covaraince matrix for random field for lambda t
  int counter = 0;
  for (int t=0; t < n_t; t++){
  ...
  for (int t = 0; t < n_t; t++){
    ...
    for(int i=0;i<n(t);i++){
      q((counter2+i),0)=(Type(1)-exp(-(ldat((counter2+i))+ldant((counter2+i)))*s((counter2+i))))*(ldat((counter2+i))/(ldat((counter2+i))+ldant((counter2+i))))*(Type(1)-pnt);
  ...
  for (int t = 0; t < n_t; t++){
    ...
    for(int i=0;i<n2(t);i++){
      ...
      p((counter4+i),1)=(Type(1)-exp(-(ldat(H_i(counter4+i))+ldant(H_i(counter4+i)))*s(H_i(counter4+i))))*(ldat(H_i(counter4+i))/(ldat(H_i(counter4+i))+ldant(H_i(counter4+i))))*(Type(1)-pnt);
