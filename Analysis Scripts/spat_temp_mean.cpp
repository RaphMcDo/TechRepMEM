...
Type objective_function<Type>::operator() ()
{
  ...
  for (int t=0; t <n_t; t++){
    nll_joint(5) -= dnorm(rand_t(t),mean_rand_t,sigma_rand_t,true);
    nll_joint(6) -= dnorm(rand_nt(t),mean_rand_nt,sigma_rand_nt,true);
  }
  ...
  SIMULATE{
    ...
    for (int t=0; t<n_t; t++){
      rand_t(t) = mean_rand_t+rnorm(Type(0.0),sigma_rand_t);
      rand_nt(t) = mean_rand_nt+rnorm(Type(0.0),sigma_rand_nt);
    }
    REPORT(rand_t);
    REPORT(rand_nt);
  }
