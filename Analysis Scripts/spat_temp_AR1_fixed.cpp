...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER(logit_phi_ar_t);
  PARAMETER(logit_phi_ar_nt);
  // spatial parameters
  ...
  Type phi_ar_t = 2*invlogit(logit_phi_ar_t)-1;
  Type phi_ar_nt = 2*invlogit(logit_phi_ar_nt)-1;
  ...
  nll_joint(5) -= dnorm(rand_t(0),mean_ar_t,sigma_rand_t,true);
  nll_joint(6) -= dnorm(rand_nt(0),mean_ar_nt,sigma_rand_nt,true);
  for (int t=1; t <n_t; t++){
    nll_joint(5) -= dnorm(rand_t(t),mean_ar_t + phi_ar_t*rand_t(t-1),sigma_rand_t,true);
    nll_joint(6) -= dnorm(rand_nt(t),mean_ar_nt + phi_ar_nt*rand_nt(t-1),sigma_rand_nt,true);
  }
  ...
  SIMULATE{
    ...
    rand_t(0) = mean_ar_t+rnorm(Type(0.0),sigma_rand_t);
    rand_nt(0) = mean_ar_nt+rnorm(Type(0.0),sigma_rand_nt);
    for (int t=1; t<n_t; t++){
      rand_t(t) = (mean_ar_t + phi_ar_t*rand_t(t-1))+rnorm(Type(0.0),sigma_rand_t);
      rand_nt(t) = (mean_ar_nt + phi_ar_nt*rand_nt(t-1))+rnorm(Type(0.0),sigma_rand_nt);
    }
    REPORT(rand_t);
    REPORT(rand_nt);
  }
